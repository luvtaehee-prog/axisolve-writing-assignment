...
2. **처리** — 브라우저가 `POST /api/generate` 호출 → Python 함수가 서버에서 Claude API 호출
...
| API 키 미설정 | "ANTHROPIC_API_KEY 가 설정되지 않았습니다" (HTTP 500) |
...
Anthropic API 호출과 인증은 전부 서버 함수 안에서 일어나며,
키는 Vercel 환경변수 `ANTHROPIC_API_KEY` 에만 존재합니다.
...
├── api/generate.py     # Vercel Python Serverless Function (Claude API 프록시)
...
4. **Settings > Environment Variables** 에 추가합니다.
   ...
   - Value: `sk-ant-` 로 시작하는 키 ([console.anthropic.com](https://console.anthropic.com) 발급)
   - 선택: `ANTHROPIC_MODEL` (기본값 `claude-sonnet-5`)
...
cp .env.example .env.local     # ANTHROPIC_API_KEY 값을 채웁니다
...
생성 버튼을 누를 때마다 Claude API가 호출되며, 기준 미달 시 자동 재시도로
